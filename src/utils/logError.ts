    ...
    if(originalException.name === 'ExitPromptError'){
        ...
        if(rfcClientError.rfcError && rfcClientError.rfcError.key){
            if(rfcClientError.rfcError.key[0] === 'Z'){
                ...
                Logger.error(`${rfcClientError.rfcError.key} ${sError}`, true);
            }else{
                sError = `${chalk.black.bgRed(rfcClientError.rfcError.key)} ${sError}`;
                if(rfcClientError.rfcError.key === "TRM_RFC_UNAUTHORIZED"){
                    sError = chalk.black.bgRed(`${chalk.black.bgRed(rfcClientError.rfcError.key)} ${_getUnauthorizedError()}`);
                }
            }
        }
